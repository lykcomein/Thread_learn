		...
		try {
			for (;;) {
				incoming.wait().handle<issue_money,
					std::function<void(issue_money const& msg)>,
						...
						[&](display_insufficient_funds const& msg) {
							...
							std::function<void(display_enter_card const& msg)>>(
								[&](display_enter_card const& msg) {
									...
									std::function<void(display_balance const& msg)>>(
										[&](display_balance const& msg) {
											{
												std::lock_guard<std::mutex> lk(iom);
												std::cout << "The balance of you account is " << msg.amount << std::endl;
											}
										},"display_balance"
