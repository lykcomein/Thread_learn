...
class atm 
{
	...
	void process_withdrawal() {
		incoming.wait().handle<withdraw_ok, 
			std::function<void (withdraw_ok const& msg)>,
			...
			std::function<void(cancel_pressed const& msg)>>(
	...
	void process_balance()
	{
		incoming.wait().handle<balance,
			std::function<void (balance const& msg)>,
			messaging::dispatcher>(
				[&](balance const& msg) {
	...
	void wait_for_action() {
		...
		incoming.wait().handle<withdraw_pressed,
			...
			std::function<void(balance_pressed const& msg) >>(
	...
	void verifying_pin()
	{
		incoming.wait()
			.handle<pin_verified,
			std::function<void(pin_verified const& msg)>,
			messaging::dispatcher>(
			...
			std::function<void(pin_incorrect const& msg)>>(
	...
	void getting_pin()
	{
		incoming.wait().handle<digit_pressed,
			std::function<void(digit_pressed const& msg)>,
			messaging::dispatcher>(
			...
			std::function<void(clear_last_pressed const& msg)>>(
	...
	void waiting_for_card()
	{
		...
		std::function<void(card_inserted const& msg)>,
			messaging::dispatcher>(
