...
public:
	CircularQueLight():_max_size(Cap + 1), _data(std::allocator<T>::allocate(_max_size)),
    ...
    //_tail_update的作用：观察push函数，可以看到是先移动_tail指针，再更新_data,
    //所以可能发生移动tail后，还未更新_data的值的情况，这时如果pop恰好要读更新后的_data的值
    //就会发生错误。所以这里使用_tail_update来做一个标记，_tail_update跟随_tail而变化
    //当push函数移动_tail后，紧接着更新_data的值，最后更新_tail_update使其等于_tail
    //这样就能保证当_tail == _tail_update时，_data的值已经被更新
    bool push(const T& val)
    {
        size_t t;
        do
        {
            t = _tail.load();  //1 //原子操作默认采用的是memory_order_seq_cst内存顺序，性能上不是最优的
            //判断队列是否满
